...
def _render_anti_pattern_page(yf: Path) -> str:
    ...
    # Summary table (HTML for consistent column widths)
    ...
    for p in patterns:
        check_id = p.get("id", "?")
        warning = p.get("warning_message", "").replace("\n", " ")
        lines.append(f'<tr><td><a href="#{check_id}">{check_id}</a></td><td>{warning}</td></tr>')
    lines.append("</tbody>")
    lines.append("</table>")
    lines.append("")
    lines.append("<hr />")
    ...
    for p in patterns:
        ...
        lines.append("<thead>")
        lines.append("<tr>")
        lines.append("<th>Services</th><th>Triggers On</th><th>Correct Patterns</th>")
        lines.append("</tr>")
        lines.append("</thead>")
        lines.append("<tbody>")
        lines.append(f"<tr><td>{svc_html}</td><td>{search_html}</td><td>{correct_html}</td></tr>")
        lines.append("</tbody>")
        lines.append("</table>")
        lines.append("")
        lines.append("<hr />")
...
def _render_standard_page(yf: Path) -> str:
    ...
    for p in principles:
        ...
        lines.append(f'<tr><td><a href="#{pid}">{pid}</a></td><td>{desc}</td></tr>')
    lines.append("</tbody>")
    lines.append("</table>")
    lines.append("")
    lines.append("<hr />")
...
def _render_policy_page(yf: Path) -> str:
    ...
    if patterns_list:
        ...
        lines.append("<table>")
        lines.append("<thead>")
        lines.append("<tr><th>Name</th><th>Description</th></tr>")
        lines.append("</thead>")
        lines.append("<tbody>")
        for pat in patterns_list:
            name = pat.get("name", "")
            desc = pat.get("description", "")
            lines.append(f"<tr><td>{name}</td><td>{desc}</td></tr>")
        lines.append("</tbody>")
        lines.append("</table>")
        lines.append("")
    ...
    if anti_patterns_list:
        ...
        lines.append("<table>")
        lines.append("<thead>")
        lines.append("<tr><th>Description</th><th>Instead</th></tr>")
        lines.append("</thead>")
        lines.append("<tbody>")
        for ap in anti_patterns_list:
            desc = ap.get("description", "")
            instead = ap.get("instead", "")
            lines.append(f"<tr><td>{desc}</td><td>{instead}</td></tr>")
        lines.append("</tbody>")
        lines.append("</table>")
        lines.append("")
    ...
    # Rules summary table (HTML)
    lines.append(f"### Checks ({len(rules)})")
    lines.append("")
    lines.append("<table>")
    lines.append("<thead>")
    lines.append("<tr>")
    lines.append('<th width="185">Check</th><th>Severity</th><th>Description</th>')
    lines.append("</tr>")
    lines.append("</thead>")
    lines.append("<tbody>")
    for rule in rules:
        ...
        lines.append(f'<tr><td><a href="#{rid}">{rid}</a></td><td>{severity}</td><td>{desc}</td></tr>')
    lines.append("</tbody>")
    lines.append("</table>")
    lines.append("")
    lines.append("<hr />")
    ...
    for rule in rules:
        ...
        if companion_resources:
            ...
            lines.append("<table>")
            lines.append("<thead>")
            lines.append("<tr><th>Resource</th><th>Name</th><th>Purpose</th></tr>")
            lines.append("</thead>")
            lines.append("<tbody>")
            for cr in companion_resources:
                res_type = _strip_api_version(cr.get("type", ""))
                name = cr.get("name", "")
                purpose = cr.get("description", "")
                lines.append(f"<tr><td>{res_type}</td><td>{name}</td><td>{purpose}</td></tr>")
            lines.append("</tbody>")
            lines.append("</table>")
            lines.append("")
